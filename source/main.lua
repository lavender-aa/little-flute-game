...
function pd.update()
	...
	drawDegree(curr_note)
	drawPitch(curr_note)

	-- play the note when a button is pressed
...
function getNoteFromInput()
	...
	for _, b in ipairs(buttons) do
		dir = dir .. (pressed(b) and "1" or "0")
	end

	-- figure out what pitch to play
	pit = 0
	if pressed(pd.kButtonB) then
		pit += 1
	end
	...
	if synthPlayer == nil then
		return { digree = 0, pitch = pit }
	end
	if dir == "0000" then
		synthPlayer:noteOff()
		return { degree = 0, pitch = pit }
	end
...
function drawDegree(note)
	-- draw the degree of the note
	local x, y = degree_center.x, degree_center.y
	local radius = 75
	if note == nil or note.degree == nil or note.degree == 0 then
		return
	else
